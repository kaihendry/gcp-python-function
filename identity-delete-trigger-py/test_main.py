...
# Mocking credentials setup
patch("google.auth.default", return_value=(MagicMock(), "test-project")).start()
patch("google.cloud.logging.Client", autospec=True).start()
patch("google.cloud.firestore.Client", autospec=True).start()

from main import firestoreUserDelete  # Import after patching
import logging
